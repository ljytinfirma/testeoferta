...
# Script de Deploy Automático - ENCCEJA Ubuntu VPS (Python 3.10)
# Execute como root: bash deploy_vps_python310.sh

echo "=== INICIANDO DEPLOY ENCCEJA VPS (Python 3.10) ==="

# 1. Atualizar sistema
echo "Atualizando sistema..."
apt update && apt upgrade -y

# 2. Instalar dependências (usando Python 3.10 que já existe)
echo "Instalando dependências..."
apt install python3 python3-pip python3-venv nginx supervisor git certbot python3-certbot-nginx -y
...
echo "Agora copie os arquivos do projeto:"
echo "cp app.py /var/www/encceja/"
echo "cp -r templates/ /var/www/encceja/"
echo "cp -r static/ /var/www/encceja/"
echo "supervisorctl start encceja"
echo ""
echo "Sistema disponível em: http://$(curl -s ifconfig.me 2>/dev/null || echo 'SEU_IP')"